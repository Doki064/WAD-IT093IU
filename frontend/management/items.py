    ...
    with st.beta_container():
        ...
        with col1:
            ...
            if choice == "id":
                ...
                response = await items.get_all(mngmt.client, limit=mngmt.limit)
                try:
                    response.raise_for_status()
                except httpx.HTTPStatusError:
                    st.error(f"Status code: {response.status_code}")
            ...
            columns = st.multiselect("Select columns to show: ", mngmt.metadata["items"])

        with col2:
            with st.beta_expander("Show item with selected column(s)", expanded=True):
                if not columns:
                    columns = mngmt.metadata["items"]
