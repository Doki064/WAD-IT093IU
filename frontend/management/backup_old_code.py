...
from aiohttp import Clientclient
...
class Management:
    ...
    @classmethod
    async def create(cls, state: clientState, client: Clientclient):
        self = Management()
        self.state = state
        self.client = client
        self.current_option = ""
            ...
            if self.current_option == "Customer":
                with col1:
                    ...
                    columns = st.multiselect(
                        "Select columns to show: ", self.customer_columns
                    )
                ...
                if choice == "id":
                    ...
                    data = customer.search_by_name(
                        self.connection, customer_name, columns
                    )
                ...
                with col2:
                    with st.beta_expander(
                        "Show customer with selected column(s)", expanded=True
                    ):
                        st.dataframe(df)
                ...
                with col1:
                    ...
                    columns = st.multiselect(
                        "Select columns to search: ", self.category_columns
                    )
                ...
                if choice == "id":
                    data = item_category.search_by_id(
                        self.connection, category_id, columns
                    )
                elif choice == "name":
                    data = item_category.search_by_name(
                        self.connection, category_name, columns
                    )
                ...
                with col2:
                    with st.beta_expander(
                        "Show category with selected column(s)", expanded=True
                    ):
                        st.dataframe(df)
                ...
                with col1:
                    ...
                    if choice == "id":
                        ...
                        shop_name = st.text_input(
                            "Input shop name (* to search all): ", value=""
                        )

                    columns = st.multiselect(
                        "Select columns to show: ", self.shop_columns
                    )
                ...
                with col2:
                    with st.beta_expander(
                        "Show shop with selected column(s)", expanded=True
                    ):
                        st.dataframe(df)
                ...
                with col1:
                    ...
                    columns = st.multiselect(
                        "Select columns to show: ", self.imports_columns
                    )
                ...
                with col2:
                    with st.beta_expander(
                        "Show import with selected column(s)", expanded=True
                    ):
                        st.dataframe(df)
                    ...
                    with st.beta_expander("Search for import details"):
                        ...
                        st.dataframe(
                            pd.DataFrame.from_records(
                                data,
                                columns=import_detail.columns_names(self.connection)
                            )
                        )
                ...
                with col1:
                    ...
                    choice = st.radio(
                        ...
                        options=[
                            'id', 'date', 'status', 'customer', 'shop', 'get all records'
                        ]
                    )
                    ...
                    if choice == "id":
                        ...
                        transaction_date = datetime.fromordinal(
                            st.date_input(
                                "Input date: ",
                                min_value=transactions.get_min_max_date(self.connection
                                                                       )[0],
                                max_value=transactions.get_min_max_date(self.connection
                                                                       )[1],
                        ...
                        transaction_status = st.radio(
                            "Transaction status: ", options=['Pending', 'Completed']
                        ).upper()
                    ...
                    columns = st.multiselect(
                        "Select columns to show: ", self.transactions_columns
                    )
                ...
                if choice == "id":
                    data = transactions.search_by_id(
                        self.connection, transaction_id, columns
                    )
                elif choice == "date":
                    data = transactions.search_by_date(
                        self.connection, transaction_date, columns
                    )
                elif choice == "status":
                ...
                with col2:
                    ...
                    with st.beta_expander("Search for transaction details"):
                        ...
                        st.dataframe(
                            pd.DataFrame.from_records(
                                data,
                                columns=transaction_detail.columns_names(self.connection)
                            )
                        )
                ...
                with col1:
                    ...
                    if choice == "id":
                        ...
                        item_name = st.text_input(
                            "Input item name (* to search all): ", value=""
                        )
                    ...
                    columns = st.multiselect(
                        "Select columns to show: ", self.item_columns
                    )
                ...
                if choice == "id":
                    ...
                    data = item.search_by_category_id(
                        self.connection, category_id, columns
                    )
                elif choice == "shop":
                ...
                with col2:
                    with st.beta_expander(
                        "Show item with selected column(s)", expanded=True
                    ):
                        st.dataframe(df)
        ...
        with st.beta_container():
            ...
            if self.current_option == "Customer":
                ...
                if st.button("Add customer"):
                    ...
                    with st.spinner("Adding customer..."):
                        if check is None:
                            ...
                            st.dataframe(
                                pd.DataFrame.from_records(
                                    data, columns=self.customer_columns
                                )
                            )
                ...
                if st.button("Add item category"):
                    check = item_category.insert(
                        self.connection, category_id, category_name
                    )
                    ...
                    with st.spinner("Adding item category..."):
                        if check is None:
                            ...
                            data = item_category.search_by_id(
                                self.connection, category_id
                            )
                            st.dataframe(
                                pd.DataFrame.from_records(
                                    data, columns=self.category_columns
                                )
                            )
                ...
                if st.button("Add shop"):
                    ...
                    with st.spinner("Adding shop..."):
                        if check is None:
                            ...
                            st.dataframe(
                                pd.DataFrame.from_records(
                                    data, columns=self.shop_columns
                                )
                            )
                ...
                quantity = st.number_input(
                    "Input item quantity: ", step=1, value=0, min_value=0
                )
                ...
                category_name = st.selectbox(
                    "Input item category: ", list(categories.values())
                )
                category_id = None
                ...
                if st.button("Add item"):
                    check = item.insert(
                        self.connection, item_id, item_name, quantity, category_id,
                        shop_id
                    )
                    ...
                    with st.spinner("Adding item..."):
                        if check is None:
                            ...
                            st.dataframe(
                                pd.DataFrame.from_records(
                                    data, columns=self.item_columns
                                )
                            )
    ...
    def show_remove(self):
        ...
        with st.beta_container():
            self.current_option = st.selectbox(
                "Select table to remove: ", tables_for_remove
            )
            ...
            if self.current_option == "Customer":
                ...
                with st.beta_expander("Remove customer(s)", expanded=True):
                    if choice == "id":
                        ...
                        selected_ids = st.multiselect(
                            "Select customer id(s): ", df["customerID"]
                        )
                        data = customer.search_by_name(self.connection, customer_name)
                    ...
                    try:
                        df = pd.concat(
                            [
                                pd.DataFrame.from_records(
                                    data, columns=self.customer_columns
                                ).loc[df["customerID"] == i] for i in selected_ids
                            ],
                            ignore_index=True
                        )
                    except ValueError:
                ...
                with st.beta_expander("Remove item category(s)", expanded=True):
                    if choice == "id":
                        ...
                        selected_ids = st.multiselect(
                            "Select category id(s): ", df["categoryID"]
                        )
                        data = item_category.search_by_name(
                            self.connection, category_name
                        )
                    ...
                    try:
                        df = pd.concat(
                            [
                                pd.DataFrame.from_records(
                                    data, columns=self.category_columns
                                ).loc[df["categoryID"] == i] for i in selected_ids
                            ],
                            ignore_index=True
                        )
                    except ValueError:
                    ...
                    try:
                        df = pd.concat(
                            [
                                pd.DataFrame.from_records(
                                    data, columns=self.shop_columns
                                ).loc[df["shopID"] == i] for i in selected_ids
                            ],
                            ignore_index=True
                        )
                    except ValueError:
                    ...
                    try:
                        df = pd.concat(
                            [
                                pd.DataFrame.from_records(
                                    data, columns=self.customer_columns
                                ).loc[df["itemID"] == i] for i in selected_ids
                            ],
                            ignore_index=True
                        )
                    except ValueError:
