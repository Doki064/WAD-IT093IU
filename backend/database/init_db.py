...
async def init_data():
    ...
    for file in files:
        ...
        try:
            ...
            break
        # await df.to_sql(con=con, name=f.stem, if_exists="append", index=False)
...
#         csv_reader = csv.DictReader(csv_file)
#         for count, row in enumerate(csv_reader):
#             if count == 0 or int(row["importation_id"]) == 0 or int(row["item_id"]) == 0: # noqa: E501
#                 pass
...
#         csv_reader = csv.DictReader(csv_file)
#         for count, row in enumerate(csv_reader):
#             if count == 0 or int(row["transaction_id"]) == 0 or int(row["item_id"]) == 0: # noqa: E501
#                 pass
